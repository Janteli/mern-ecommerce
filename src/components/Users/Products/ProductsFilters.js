...
if(category){
  ...
  if(brand){
    productUrl = `${baseURL}&brand=${brand}`
  } 
  // check size
  if (size) {
    productUrl = `${baseURL}&size=${size}`;
  }
   // check size
   if (price) {
    productUrl = `${baseURL}&price=${price}`;
  }
    // check color
    if (color) {
      productUrl = `${baseURL}&color=${color?.name}`;
    }
  ...
  }, [dispatch, productUrl, color, size, price]);
