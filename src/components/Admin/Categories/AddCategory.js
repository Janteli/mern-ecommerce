...
import { createCategoriesAction } from "../../../redux/slice/categories/categoriesSlice";
...
export default function CategoryToAdd() {
  ...
  const handleOnChange = (e) => {
    ...
    // files
    const [file, setFile] = useState(null);//removed empty array, coz not handling multiple 
    const [fileErr, setFileErr] = useState(null); //here also
    // file handlechange
    const fileHandleChange = (event) => {
      // console.log(event);
      // const newFile = Array.from(event.target.files);
      const newFile = event.target.files[0];
      // console.log("arrayFrom", newFiles);
      // validation
