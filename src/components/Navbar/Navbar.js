...
import { getCartItemsFromLocalStorageAction } from "../../redux/slice/cart/cartSlices";
import { logoutAction } from "../../redux/slice/users/userSlice";
import { fetchCouponsAction } from "../../redux/slice/coupon/couponSlice";
...
export default function Navbar() {
  ...
  // get login user from locatstorage

  const user = JSON.parse(localStorage.getItem("userInfo"));
  // console.log(user?.userFound?.isAdmin);
          ...
          {!currentCoupon?.isExpired && <div className="bg-yellow-500">
            <div className="mx-auto flex h-10 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
              <p style={{textAlign:'center', width:'100%'}} className="flex-1 text-center text-sm font-medium text-white lg:flex-none">
                {currentCoupon ? `Use ${currentCoupon?.code} as a coupon to get - ${currentCoupon?.discount}% discount. Hurry up!!! valid till ${new Date(currentCoupon.endDate).toLocaleDateString()}. ${currentCoupon?.daysLeft} ` : "No Flash sale at the moment"}
              </p>
            </div>
          </div>}
